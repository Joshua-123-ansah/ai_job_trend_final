...
d3.csv(CSV_PATH, (row) => ({
  ...
  .then((rows) => {
    ...
    const jobCreationOnly = summary.filter((d) => d.trend === "Job Creation");

    state.summary = jobCreationOnly;
    state.jobsByIndustry = jobsByIndustry;

    const yExtent = d3.extent(jobCreationOnly, (d) => d.jobChangePct);
    ...
    sizeScale.domain([0, d3.max(jobCreationOnly, (d) => d.absJobChangePct) || 20]);
...
function setupDefs() {
  ...
  gradient
    .append("stop")
    .attr("offset", "0%")
    .attr("stop-color", "rgba(46,204,113,0.25)");

  gradient
    .append("stop")
    .attr("offset", "100%")
    .attr("stop-color", "rgba(46,204,113,0.12)");
